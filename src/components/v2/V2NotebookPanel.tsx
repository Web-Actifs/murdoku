      ...
      {!state.audited ? (
        <>
          ...
          <button
            type="button"
            onClick={runAudit}
            className="mt-2 rounded-[var(--radius-sm)] border-2 border-[var(--color-accent)] px-3 py-1.5 text-sm font-bold text-[var(--color-accent)]"
          >
            {text.t('v2.notebook.open')}
          </button>
        </>
      ) : marks === 0 ? (
        <p className="mt-1 text-xs text-[var(--color-text-muted)]">{text.t('v2.notebook.empty')}</p>
      ) : (
        <>
          ...
          <ul className="mt-2 flex flex-col gap-1">
            {VERDICTS.filter((verdict) => audit.counts[verdict] > 0).map((verdict) => (
              <li key={verdict} className="flex items-start gap-2 text-xs">
          ...
          <p className="mt-2 text-xs font-semibold" style={{ color: audit.disciplined ? 'var(--color-success)' : 'var(--color-text-muted)' }}>
